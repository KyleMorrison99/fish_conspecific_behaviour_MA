...
## lnRR intercept only model (lnRR)
```{r}
# Model
...
filter(!dosage_unit %in% c("nm", "um", "nM")) %>%
  mutate(dosage = as.numeric(dosage),
   ...
   filter(!is.na(dosage_convert_ugL)) %>% 
    mutate(log_dosage_convert_ugL = log(dosage_convert_ugL)) # NOTE TO SELF THAT THIS NEEDS ADJSUTED FOR SOME TOXICITY METRIC - at the moment it does not show much 
...
## Moderate by pesticide chemical class 
...
leave_study_out_model <- list()
VCV_leave_study_out <- list()
...
VCV_leave_study_out[[i]] <- impute_covariance_matrix(vi = data_study_leave$lnRRv, cluster = data_study_leave$study_id, r = 0.5) 
### model fitting
leave_study_out_model[[i]] <- rma.mv(yi = lnRR,V = VCV_leave_study_out[[i]], 
                                   random = list(~1 | study_id,
                                                   ~1 | assay_id,
                                                   ~1 | species_latin,
                                                   ~1 | pesticide),
...
results.leave_study_out_model <-as.data.frame(cbind(
                                           sapply(leave_study_out_model, function(x) summary(x)$beta), # extract the beta coefficient from all models
                                           sapply(leave_study_out_model, function(x) summary(x)$se), # extract the standard error from all models
                                           sapply(leave_study_out_model, function(x) summary(x)$zval),  # extract the z value from all models
                                           sapply(leave_study_out_model, function(x) summary(x)$pval), # extract the p value from all models
                                           sapply(leave_study_out_model, function(x) summary(x)$ci.lb), # extract the lower confidence interval for all models
                                           sapply(leave_study_out_model, function(x) summary(x)$ci.ub))) # extract the upper confidence interval for all models

colnames(results.leave_study_out_model)=c("Estimate", "SE", "zval", "pval", "ci.lb", "ci.ub") # change column names 

# Add the study_id as the index for the plot
results.leave_study_out_model$study_id <- unique(data$study_id)

# Display the results table
kable(results.leave_study_out_model) %>%
  kable_styling("striped", position = "left") %>%
  scroll_box(width = "100%", height = "500px")


leave_one_study_out_forest_plot <- ggplot(results.leave_study_out_model, 
...
print(leave_one_study_out_forest_plot)


results.leave_study_out_model %>% 
...
leave_species_out_mod <- list()
VCV_leave_species_out <- list()
...
for(i in 1:length(levels(data$leave_species))){
  ...
  VCV_leave_species_out[[i]] <- impute_covariance_matrix(vi = data_species_leave$lnRRv, cluster = data_species_leave$study_id, r = 0.5)
  
    # Model fitting
    leave_species_out_mod[[i]] <- rma.mv(yi = data_species_leave$lnRR,
                                         V = VCV_leave_species_out[[i]],
                                         random = list(~1 | study_id,
                                                       ~1 | assay_id,
                                                       ~1 | species_latin,
                                                       ~1 | pesticide),
...
results.leave_species_out_model <- as.data.frame(cbind(
  sapply(leave_species_out_mod, function(x) summary(x)$beta), # extract the beta coefficient from all models
  sapply(leave_species_out_mod, function(x) summary(x)$se),   # extract the standard error from all models
  sapply(leave_species_out_mod, function(x) summary(x)$zval), # extract the z value from all models
  sapply(leave_species_out_mod, function(x) summary(x)$pval), # extract the p value from all models
  sapply(leave_species_out_mod, function(x) summary(x)$ci.lb),# extract the lower confidence interval for all models
  sapply(leave_species_out_mod, function(x) summary(x)$ci.ub) # extract the upper confidence interval for all models
))

colnames(results.leave_species_out_model) <- c("Estimate", "SE", "zval", "pval", "ci.lb", "ci.ub") # change column names
results.leave_species_out_model$species_latin <- unique(data$species_latin)

# Display the results table
kable(results.leave_species_out_model) %>%
  kable_styling("striped", position = "left") %>%
  scroll_box(width = "100%", height = "500px")

leave_one_species_out_forest_plot <- ggplot(results.leave_species_out_model, 
...
print(leave_one_species_out_forest_plot)
```
...
# Initialize lists to contain the model estimates and VCV matrices
leave_pesticide_out_mod <- list()
VCV_leave_pesticide_out <- list()
...
for(i in 1:length(levels(data$leave_pesticide))){
  ...
  # Create a VCV matrix for the subset data
  VCV_leave_pesticide_out[[i]] <- impute_covariance_matrix(vi = data_pesticide_leave$lnRRv, cluster = data_pesticide_leave$study_id, r = 0.5)
  
  # Model fitting
  leave_pesticide_out_mod[[i]] <- rma.mv(yi = data_pesticide_leave$lnRR,
                                         V = VCV_leave_pesticide_out[[i]],
                                         random = list(~1 | study_id,
                                                       ~1 | assay_id,
                                                       ~1 | species_latin,
                                                       ~1 | pesticide),
...
results.leave_pesticide_out_model <- as.data.frame(cbind(
  sapply(leave_pesticide_out_mod, function(x) summary(x)$beta), # extract the beta coefficient from all models
  sapply(leave_pesticide_out_mod, function(x) summary(x)$se),   # extract the standard error from all models
  sapply(leave_pesticide_out_mod, function(x) summary(x)$zval), # extract the z value from all models
  sapply(leave_pesticide_out_mod, function(x) summary(x)$pval), # extract the p value from all models
  sapply(leave_pesticide_out_mod, function(x) summary(x)$ci.lb),# extract the lower confidence interval for all models
  sapply(leave_pesticide_out_mod, function(x) summary(x)$ci.ub) # extract the upper confidence interval for all models
))

colnames(results.leave_pesticide_out_model) <- c("lnRR", "SE", "zval", "pval", "ci.lb", "ci.ub") # change column names
results.leave_pesticide_out_model$pesticide_chemical_class <- unique(data$pesticide_chemical_class)

# Display the results table
kable(results.leave_pesticide_out_model) %>%
  kable_styling("striped", position = "left") %>%
  scroll_box(width = "100%", height = "500px")

leave_one_pesticide_out_forest_plot <- ggplot(results.leave_pesticide_out_model, 
  ...
  theme(legend.position = "none",
        ...
        plot.caption = element_text(size = 10, color = "gray10", face = "italic"),
        axis.text.y = element_text(size = 8)) +
  geom_text(aes(label = round(lnRR, 2)), size = 4, color = "gray10", nudge_x = 0.3) +
  labs(caption = "The value in each point is the estimate excluding one pesticide at a time",
       x = "Pesticide Chemical Class",
       y = "lnRR") +
  coord_flip()

print(leave_one_pesticide_out_forest_plot)


```


## Leave imputed SD out
