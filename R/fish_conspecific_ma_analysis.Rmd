...
lnRRVCV2 <- metafor::vcalc(vi = lnRRv, cluster = exp2_id, obs = assay2_id, data = data, rho = 0.5) # Sensitivity analysis with alternative VCV
...
#------------------------------SMDH------------------------------#
SMDHVCV <- metafor::vcalc(vi = SMDHv, cluster = exp_id, obs = assay_id, data = data, rho = 0.5)
```
...
## Add *exp_id* as a random effect to the above better model (that used *assau_id* and *study_id* as random effets) (lnRR)
```{r}
...
Adding study *study_id* as a random effect did not improve model quality

## Add *species* as a random effect to the above better model (that used *assay_id* and *study_id* as random effets) (lnRR)
```{r}
...
## Add *phylogeny* as a random effect to the above better model (that used *assay_id* , *study_id* and *species* as random effects) (lnRR)
```{r}
...
anova(mod_lnRR.assay_id.study_id.species, mod_lnRR.assay_id.study_id.species.phylogeny)
...
## Add *pesticide* as a random effect to the above better model (that used *assay_id* , *study_id*, *species* and *phylogeny* as random effects) (lnRR)
```{r}
mod_lnRR.assay_id.study_id.species.phylogeny.pesticide <- rma.mv(yi = lnRR,
                                                       V = lnRRVCV,
                                                       random = list(~1 | study_id,
                                                                     ~1 | assay_id,
                                                                     ~1 | species_latin,
...
summary(mod_lnRR.assay_id.study_id.species.phylogeny.pesticide)

anova(mod_lnRR.assay_id.study_id.species.phylogeny, mod_lnRR.assay_id.study_id.species.phylogeny.pesticide)
...
lnRR_intercept_only_robust <- robust(lnRR_intercept_only, data$study_id)

summary(lnRR_intercept_only_robust)

i2_ml(lnRR_intercept_only)
  
```

### Intercept only model lnRR orchard plot
```{r}
orchaRd::orchard_plot(lnRR_intercept_only,
             group = "behaviour_measured_standardised", 
             xlab = "lnRR",
             data = data,
             k.pos = "right",
             colour = FALSE,
...
lnCVR_intercept_only <- rma.mv(yi = lnCVR,
                               ...
                               random = list(~1 | study_id,
                                       ...
                                       #      ~1 | species_latin,
                                         #    ~1 | phylogeny),
                               mods = ~ data$behaviour_measured_standardised -1,
                               method = "REML",
                               test = "t", 
                               data = data,
                               sparse = T)
...
lnCVR_intercept_only_robust <- robust(lnCVR_intercept_only, data$study_id)

summary(lnCVR_intercept_only_robust)
...
data <- data[!is.na(data$lnRRv),]
lnRRVCV2 <- metafor::vcalc(vi = lnRRv, cluster = study_id, obs = assay_id, data = data2, rho = 0.5)


null_mod_lnRR <- rma.mv(yi = lnRR, 
                        V = lnRRVCV, 
              mods = ~ data2$behaviour_measured_standardised - 1,
                        method = "ML", 
                        test = "t", 
                        data = data2,
                        sparse = TRUE )

orchaRd::orchard_plot(null_mod_lnRR,
                      mod = "behaviour_measured_standardised",
                      group = "study_id", 
                      xlab = "lnRR",
                      alpha = 0.7,
...
data3 <- data[!is.na(data$lnCVRv),]
lnRRVCV <- metafor::vcalc(vi = lnRRv, cluster = exp_id, obs = assay_id, data = data3, rho = 0.5)


null_mod_lnCVR <- rma.mv(yi = lnCVR, 
                        V = lnRRVCV, 
                        mods = ~ data3$behaviour_measured_standardised - 1,
                        method = "ML", 
                        test = "t", 
                        data = data2,
                        sparse = TRUE )

orchaRd::orchard_plot(null_mod_lnCVR,
                      mod = "behaviour_measured_standardised",
                      group = "study_id", 
                      xlab = "lnCVR",
                      alpha = 0.7,
