...
theme(legend.position = "none",
        axis.text = element_text(color = "black"),
        axis.title = element_text(face = "bold", color = "black"),
        plot.caption = element_text(size = 10, color = "gray10", face = "italic")) +  geom_text(aes(label = as.character(n)), size = 4, color = "gray10") +
  labs(caption = "The value in each cell is the number of effect sizes",
       x = "Behaviour Measured",
       y = "Species") +
...
## Plot imputed standard deviations
```{r}
...
## Calculate effect size estimates (lnRR & lnCVR)
```{r}
# Filter data for NA's
data <- data %>%
  filter(!is.na(treatment_mean) & !is.na(treatment_sd) & !is.na(treatment_n) &
...
# Calculate precision for each effect size estimate
lnRR$precision <- 1 / sqrt(lnRR$yi)
lnCVR$precision <- 1 / sqrt(lnCVR$yi)
SMD$precision <- 1 / sqrt(SMD$yi)
SMDH$precision <- 1 / sqrt(SMDH$yi)
lnVR$precision <- 1/ sqrt(lnVR$yi)

# Merge each of the effect size stim
effect_size_metrics_set <- data.frame(lnRR = lnRR$yi, lnRRv = lnRR$vi, lnVR = lnVR$yi, lnVRv = lnVR$vi, lnCVR = lnCVR$yi, lnCVRv = lnCVR$vi, SMD = SMD$yi, SMDv = SMD$vi, SMDH = SMDH$yi, SMDHv = SMDH$vi)

data <- cbind(data, effect_size_metrics_set) # bind_cols()

# s
effect_size_metrics_set_kable <- data[,which(colnames(data) %in% c("study_id", "species_english", "pesticide", "lnRR", "lnRRV", "lnVR", "lnVRV", "lnCVR", "lnCVRV", "SMD", "SMDV", "SMDHV"))] %>% dfround(digits = 3)
